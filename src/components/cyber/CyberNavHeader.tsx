...
import { Volume2, VolumeX, Menu, X, Github, Linkedin, Mail, Globe, ExternalLink } from 'lucide-react';
import { SiLeetcode } from 'react-icons/si';
import { cyberAudio } from '../../utils/CyberAudioEngine';

interface CyberNavHeaderProps {
  onNavigate: (sectionId: string) => void;
}

export const CyberNavHeader: React.FC<CyberNavHeaderProps> = ({ onNavigate }) => {
      ...
      <header className="fixed top-0 left-0 right-0 z-50 px-6 py-5 flex items-center justify-between pointer-events-none">
        ...
        <div className="flex items-center gap-3 pointer-events-auto">
          ...
          <button
            ...
            className="px-3.5 h-10 rounded-xl bg-[#161617]/90 border border-[#26262D] hover:border-[#00D4FF] text-gray-200 hover:text-white flex items-center gap-2 backdrop-blur-md transition-all shadow-md active:scale-95 group"
          >
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 my-auto py-12">
              <div className="flex flex-col gap-6">
                {[
                  { id: 'landing', label: '01 // HOME', desc: 'Introduction & 3D Hero' },
                  { id: 'about', label: '02 // ABOUT & SKILLS', desc: 'NLP Research, AI & Tech Stack' },
                ].map((item) => (
                  <div
                    key={item.id}
              ...
              <div className="flex flex-col gap-6">
                {[
                  { id: 'work', label: '03 // WORK & VENTURES', desc: 'Ninzae, Store & Shipped Products' },
                  { id: 'contact', label: '04 // CONTACT & SOCIALS', desc: 'Get in touch & Connect' },
                ].map((item) => (
                  <div
                    key={item.id}
                    onClick={() => handleNavClick(item.id)}
